...
class TaxiBillViewModel {
    ...
    var isrequiredtoshowFareBreakUpView = false


    init(taxiRideInvoice: TaxiRideInvoice?,taxiRide: TaxiRidePassenger?,isFromClosedRidesOrTransaction: Bool, isRequiredToInitiatePayment: Bool? ) {
    ...
    func isPaymentCompleted() -> Bool{
        if taxiRideInvoice?.status == TaxiRideInvoice.INVOICE_STATUS_CAPTURED_FROM_USER || taxiRideInvoice?.status == TaxiRideInvoice.INVOICE_STATUS_DEBITED_FROM_USER{
            return true
        }else{
           return false
        }
    ...
    func getPendingBillForCurrentTrip(handler: @escaping(_ taxiPendingBill: TaxiPendingBill?)->()){
        TaxiPoolRestClient.getTaxiPendingBill(id: taxiRide?.id ?? 0) { (responseObject, error) in
            if responseObject != nil && responseObject!["result"] as! String == "SUCCESS" {
                if let taxiPendingBill = Mapper<TaxiPendingBill>().map(JSONObject: responseObject!["resultData"]){
                    handler(taxiPendingBill)
                }
            }else{
                handler(nil)
            }
    ...
    func getAllRentalStopPoints(handler : @escaping(_ responseError: ResponseError?,_ error: NSError? )-> ()) {
        guard let taxiRidePassenger = self.taxiRide, taxiRidePassenger.tripType == TaxiRidePassenger.TRIP_TYPE_RENTAL,let taxiGroupId = taxiRidePassenger.taxiGroupId else {
            handler(nil,nil)
            return
        }
