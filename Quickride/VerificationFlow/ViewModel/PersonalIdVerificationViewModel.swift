...
class PersonalIdVerificationViewModel {
    ...
    func launchFaceCaptureScreen(verificationType: String, viewController: UIViewController) {
        ...
        let completionHandler:(_ error:HVError?,_ result:HVResponse?, _ viewController:UIViewController)->Void = {error, result, vcNew in
            vcNew.dismiss(animated: false, completion: nil)
            if error != nil {
                self.delegate?.documentCaptureFailed(errorMessage: error!.getErrorMessage())
            } else {
                if let result = result, let imageUri = result.imageUri {
    ...
    func makeFaceMatchCallForFrontDocImage(verificationType: String) {
        ...
        let completionHandler:(_ error:HVError?,_ result:HVResponse?)->Void = {error,result in
        ...
        HVNetworkHelper.makeFaceMatchCall(faceUri: faceImageUri!, documentUri: frontSideDocImageUri!, parameters: parameters, headers: nil, completionHandler: completionHandler)
    }
    ...
    func makeOCRCallForFrontDoc(verificationType: String) {
        ...
        let completionHandler:(_ error:HVError?,_ result:HVResponse?)->Void = {error,result in
            ...
            } else if let result = result, result.apiResult!["status"] as? String == "success", let responseData = Mapper<PersonalIdVerificationResultData>().mapArray(JSONObject: result.apiResult!["result"]), !responseData.isEmpty {
        ...
        HVNetworkHelper.makeOCRAPICall(endpoint: HyperVergeUtils.shared.getEndpoint(verificationType: verificationType), documentUri: frontSideDocImageUri!, parameters: parameters, headers: nil, completionHandler: completionHandler)
    ...
    private func makeOCRCallForBackDoc(verificationType: String) {
        ...
        let handler:(_ error:HVError?,_ result:HVResponse?)->Void = {error,result in
            
            if error != nil {
                self.delegate?.OCRCallFailed(errorMessage: error!.getErrorMessage())
            } else if let apiresult = result?.apiResult, apiresult["status"] as? String == "success", let responseData = Mapper<PersonalIdVerificationResultData>().mapArray(JSONObject: apiresult["result"]), !responseData.isEmpty {
        ...
        HVNetworkHelper.makeOCRAPICall(endpoint: HyperVergeUtils.shared.getEndpoint(verificationType: verificationType), documentUri: backSideDocImageUri!, parameters: parameters, headers: nil, completionHandler: handler)
