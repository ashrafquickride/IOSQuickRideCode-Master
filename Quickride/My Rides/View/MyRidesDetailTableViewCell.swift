//
//  MyRidesDetailTableViewCell.swift
//  Quickride
//
...
protocol MyRidesDetailTableViewCellDelegate: class {
...
class MyRidesDetailTableViewCell: UITableViewCell {
    //MARK: Outlets
    ...
    @IBOutlet weak fileprivate var rideNotificationView: UIView!
    @IBOutlet weak fileprivate var rideNotificationCountLabel: UILabel!
    @IBOutlet weak fileprivate var rideNotificationImageView: UIImageView!
    @IBOutlet weak fileprivate var rideRequestInfoLabel: UILabel!
    @IBOutlet weak fileprivate var noMatchLabel: UILabel!
    @IBOutlet weak fileprivate var shareRideButton: UIButton!
    @IBOutlet weak fileprivate var noMatchView: UIView!
    @IBOutlet weak fileprivate var matchRiderView: UIView!
    @IBOutlet weak fileprivate var matchedUserDescriptionLabel: UILabel!
    @IBOutlet weak fileprivate var matchRiderCountLabel: UILabel!
    ...
    @IBOutlet weak fileprivate var matchedRideViewWidthConstraint: NSLayoutConstraint!
    ...
    @IBOutlet weak fileprivate var firstMatchedUserImageView: CircularImageView!
    @IBOutlet weak fileprivate var seconMatchedUserImageView: CircularImageView!
    @IBOutlet weak fileprivate var thirdMatchedUserImageView: CircularImageView!
    ...
    func setRideStartDateForCardView(ride: Ride) {
        dayTitleForNextRideLabel.isHidden = false
    ...
    func configureMatchingListForMyRides(ride: Ride, section: Int, row: Int,viewController : UIViewController) {
        if section == 3 || section == 4 , row == 0 || row == 1 {
    ...
    @IBAction func shareRideButtonTapped(_ sender: UIButton) {
        JoinMyRide().prepareDeepLinkURLForRide(rideId: StringUtils.getStringFromDouble(decimalNumber: myRideDetailTableViewCellModel.ride?.rideId), riderId: StringUtils.getStringFromDouble(decimalNumber: myRideDetailTableViewCellModel.ride?.userId), from: ride?.startAddress ?? "", to: ride?.endAddress ?? "", startTime: myRideDetailTableViewCellModel.ride?.startTime ?? 0, vehicleType: (myRideDetailTableViewCellModel.ride as? RiderRide)?.vehicleType ?? "", viewController: parentViewController,isFromTaxiPool: false)
    }
    
    @IBAction func overflowButtonTapped(_ sender: UIButton) {
        delegate?.rideEditButtonTapped(ride: self.ride,rideParticipants: myRideDetailTableViewCellModel.joinedRideParticipant, senderTag: overflowButton.tag,taxiShareRide: self.taxiShareRide, dropDownView: overflowButton.plainView)
    }
...
extension MyRidesDetailTableViewCell {
    func checkStatusInfo(for ride: Ride) {
        isMatchParticipantButtonHidden = false
        rideStatusLabel.text = ""
        if ride.rideType == Ride.REGULAR_PASSENGER_RIDE || ride.rideType == Ride.PASSENGER_RIDE {
            if let passengerRide = ride as? PassengerRide {
                handleJoinedParticipant(riderRideId: passengerRide.riderRideId)
            }
            
            if let passengerRide = ride as? PassengerRide, let riderRideStatus = RideManagementUtils.getRiderRideStatusForPassenger(ride: passengerRide), riderRideStatus == Ride.RIDE_STATUS_STARTED {
                ...
                } else if Ride.RIDE_STATUS_REQUESTED == ride.status {
                    rideRequestInfoLabel.textColor = UIColor(netHex: 0x797979)
                    rideRequestInfoLabel.text = Ride.RIDE_STATUS_SCHEDULED.uppercased()
                } else if Ride.RIDE_STATUS_SCHEDULED == ride.status, let passengerRide = ride as? PassengerRide {
                ...
                } else if Ride.RIDE_STATUS_STARTED == ride.status, let passengerRide = ride as? PassengerRide {
                ...
                } else if Ride.RIDE_STATUS_DELAYED == ride.status, let passengerRide = ride as? PassengerRide {
    ...
    func setRideStatusView(status: String) {
        ...
        dayTitleForNextRideLabel.isHidden = true
...
extension MyRidesDetailTableViewCell: MyRideDetailTableViewCellModelDelegate {
...
extension MyRidesDetailTableViewCell : MyRidesCacheListener {
