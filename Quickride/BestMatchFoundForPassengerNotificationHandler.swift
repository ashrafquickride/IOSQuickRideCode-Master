//
//  BestMatchFoundForPassengerNotificationHandler.swift
//  Quickride
//
...
class BestMatchFoundForPassengerNotificationHandler: NotificationHandler {
    ...
    override func handlePositiveAction(userNotification: UserNotification, viewController: UIViewController?) {
        ...
        if let notification = bestMatchAlertNotification, let rideId = Double(notification.rideId ?? ""){
            ...
            RouteMatcherServiceClient.getMatchingRider(riderRideId: rideId, passengerRideId: rideObj.rideId, targetViewController: viewController,complitionHandler: { (responseObject, error) -> Void in
                ...
                if let responseObject = responseObject, responseObject["result"] as? String == "SUCCESS", let resultData = responseObject["resultData"] {
                    if let matchedRider = Mapper<MatchedRider>().map(JSONObject: resultData){
                        self.moveToRideDetailView(ride: rideObj, matchedUser: matchedRider,viewController: viewController)
                    }
                }
            })
        }
...
struct BestMatchAlertNotification: Mappable {
    
    var rideMatchAlertId: String?
    var userId = 0.0
    var rideId: String?
    ...
    mutating func mapping(map: Map) {
        self.rideMatchAlertId <- map["rideMatchId"]
